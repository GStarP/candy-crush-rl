...
def inference_game_result(game_state) -> bool:
    """
    推断己方是否获胜
    占领格子更多的队伍获胜，如果平局直接返回 False
    """
    team_occupy_count = {TeamColor.RED: 0, TeamColor.BLUE: 0}
    for row in game_state["map"]:
        for grid in row:
            if grid["ownership"] in (TeamColor.RED, TeamColor.BLUE):
                team_occupy_count[grid["ownership"]] += 1
    if team_occupy_count[TeamColor.RED] > team_occupy_count[TeamColor.BLUE]:
        return True if game_state["my_player"]["team"] == TeamColor.RED else False
    elif team_occupy_count[TeamColor.RED] < team_occupy_count[TeamColor.BLUE]:
        return True if game_state["my_player"]["team"] == TeamColor.BLUE else False
    else:
        return False
